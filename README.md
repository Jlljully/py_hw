...
| Вопрос  | Ответ |
| ------------- | ------------- |
| Какое значение будет присвоено переменной `c`?  | Выдает ошибку, т.к. строковая и интеджер переменная |
| Как получить для переменной `c` значение 12?  | Нужно переменную а задать как строковую: a = '1' или type(str(a)) |
| Как получить для переменной `c` значение 3?  | Наоборот нужно переменную b сделать интеджер: b = 2 или type(int(b))  |
